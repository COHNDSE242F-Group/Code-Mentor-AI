    ...
    const fetchStudent = async () => {
      try {
        ...
        setForm({
          ...
          password: '', // leave empty for security; fill only if admin wants to change
        });
      } catch (err) {
        console.error(err);
      } finally {
      ...
      <Card>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input name="batch_id" value={form.batch_id ?? ''} onChange={handleChange} className="w-full mt-1 p-2 border rounded bg-white" />
          </div>
